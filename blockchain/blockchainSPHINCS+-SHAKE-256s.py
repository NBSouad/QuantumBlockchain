...
class Blockchain:
    ...
    def verify_transaction_signature(self, sender_public_key, signature, transaction):
        signer = oqs.Signature("SPHINCS+-SHAKE-256s-simple")
        start_time = time()
        message = str(transaction).encode('utf8')
        #signature = signer.sign(message)
        signer_public_key =  binascii.unhexlify(sender_public_key)

        try:
            signer.verify(message, binascii.unhexlify(signature), signer_public_key)
    ...
    def valid_proof(transactions, last_hash, nonce, difficulty=MINING_DIFFICULTY):
        ...
        shake_hash.update(guess)
        guess_hash = shake_hash.read(32).hex()
    ...
    def hash(block):
        ...
        shake_hash = SHAKE256.new()
        shake_hash.update(block_string)
        return shake_hash.read(32).hex()
...
def verify_transaction():
    ...
    response = {
        'is_valid': is_valid,
        'verify_time_mseconds': verify_time * 1000
    }
